...
def prepare_date_rule_checks(df_export, date_rules):
    ...
    # Episode_Date in export_long_clean.csv is DD/MM/YYYY
...
def get_variable_comparison_keys(
    ...
    - If both target and reference are Longitudinal,
      compare within the same patient episode.
    - A Basic target compared against a Longitudinal
    ...
    # Basic reference:
    # one patient-level value is used for all target values of that patient
    if reference_form_type == "basic":
        return ["PID"]

    # Both variables longitudinal:
    # compare values belonging to the same episode
    if (
...
def not_after_variable(
    ...
    Matching is determined automatically:
    - reference Basic -> PID
    - target Longitudinal + reference Longitudinal -> PID + Episode
    """
...
def apply_date_rules(
    ...
    """
    Applies all temporal plausibility rules.

    df:
        Prepared date-rule working table.

    df_export:
        Full export, required so that comparison variables can
        be retrieved even when their values are not part of the
        current target rows.

    date_rules:
        Date-rule metadata table.
    """
