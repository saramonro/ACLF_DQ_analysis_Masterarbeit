...
# compare source vs target. accept exactly equal values.
# write only evaluated rows
# write mismatches to violations

Inputs: export long
Data Dictionary -> to check form type 
'''
...
def create_value_lookup(export):
    """
    Creates a   lookup table for PID x episode_date x source_id.

    If repeated identical values exist, they are collapsed. if different values exist for the same key, the value is marked as MULTIPLE_VALUES.
    Repeatable instances not supported at this time.
    """
...
def create_longitudinal_lookup(export):
    """
    Creates a dictionary:
    source_id -> unique PID x episode_date instances where this source_id exists.
    """
...
def run_import_conformance_checks(import_rules, value_lookup, longitudinal_lookup,form_type_lookup, basic_lookup):
    violations = []
    summary_rows = []
    ...
    for _, rule in import_rules.iterrows():
        ...
        target_instances = longitudinal_lookup.get(target_source_id)

        if target_instances is None or target_instances.empty:
            summary_rows.append({
                "rule_id": rule_id,
        ...
        for _, instance in target_instances.iterrows():

            pid = instance["PID"]
            episode_date = instance["episode_date"]
...
def main():
    ...
    basic_lookup = (
    create_basic_value_lookup(
        export,
        basic_source_ids,
    )
)
    target_ids = set(import_rules["target_source_id"])

    target_export = export[export["source_id"].isin(target_ids)].copy()
    longitudinal_lookup = create_longitudinal_lookup(target_export)

    (violations_df, summary_df) = run_import_conformance_checks(
        import_rules=import_rules,
        value_lookup=value_lookup,
        basic_lookup=basic_lookup,
        longitudinal_lookup=longitudinal_lookup,
