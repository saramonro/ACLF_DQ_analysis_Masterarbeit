...
from config import RAW_EXPORT_PATH as DATA_PATH
...
def create_basic_long(df, basic_ids_in_data):
    ...
    # Vectorized presence flag: transform("max") over a boolean column uses the
    # fast Cython path and avoids the memory blow-up of a Python-lambda transform.
    basic_long["has_any_value"] = (
        basic_long["source_value"]
        .notna()
        .groupby([basic_long["PID"], basic_long["source_id"]])
        .transform("max")
        .astype(bool)
    )
...
def create_longitudinal_long(df, longitudinal_ids_in_data):
    ...
    longitudinal_long["has_any_value"] = (
        longitudinal_long["source_value"]
        .notna()
        .groupby([longitudinal_long[col] for col in long_key])
        .transform("max")
        .astype(bool)
    )
