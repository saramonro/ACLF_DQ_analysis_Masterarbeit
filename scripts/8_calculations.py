...
'''
This module checks Calculation Conformance based on Behavioural metadata. It requires 3 metadata objects, which describes:
 - calculation instances: identifies target variables and maps them to their formulas
  - calculation formulas: describes formulas which can be reused by multiple instances 
   ...
   The script retrieves the target variables, input variables and formulas from the metadata, and the recorded target and input values from the export.
   Re-calculates the expected value from recorded input values + formulas
   Compares it to recorded target value.
   Slight vaiations are accepted based on metadata object. 
'''
